...
#-----------------------------------------#
# Scenario 1: Only Segment A reassorts    #
#-----------------------------------------#

## Procedure Summary:
### 1. simulate sets of trees, fixed parameters, constant expected pop size (lambda = mu + psi)
### 2. simulate alignments: seg A, B: len=1e3, evol rate=1e-5
### 3. One inference scheme
...
foreach(task.no=1:nrep, .combine=rbind, .packages=c("SimReassort","dplyr","stringr","ape"),
  ...
  write.table(as.matrix(tipdates), paste0(dir.path,"tipdates",task.no,".txt"),
	...
	## extract key trees: 1. segA, 2. segB, 3. FIT
  evol.rate <- 5e-3
  ...
  ##############################
	...
	for (k in seq_along(trees_nwk[1:2])) {
	...
	system("source ~/program/miniconda3/etc/profile.d/conda.sh")	
	system("conda activate my_env")
